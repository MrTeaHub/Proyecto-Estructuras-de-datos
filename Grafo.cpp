...
int Vertice<T>::getWeight(T &value)
{
    for (int i = 0; i < connectedTo.size(); i++)
    {
        ...
        if (temp->to->data.getNombre() == value.getNombre())
            return connectedTo[i]->weight;
    }
    return NULL;
}
...
void Grafo<T>::addAristaNoDireccional(const T &from, const T &to, int weight)
{
    ...
    if (!fromVertice)
        fromVertice = addVertice(from);
    Vertice<T> *toVertice = getVertice(to);
    if (!toVertice)
        toVertice = addVertice(to);
...
Vertice<T> *Grafo<T>::getVertice(const T &value)
{
    ...
    {
        if (listaVertices[i]->data.getNombre() == value.getNombre())
...
int Grafo<T>::obtenerPosicionVertice(Vertice<T> *vertice)
{
    for (int i = 0; i < listaVertices.size(); i++)
    {
        if (listaVertices[i]->data.getNombre() == vertice->data.getNombre())
            return i;
    }
}
